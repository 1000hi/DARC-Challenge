    ...
    def _compare_date_gt_anon(self):
        ...
        for idx in range(self._anon_trans.shape[0]):
            ...
            try:
                ...
                print("GT_DAY_BUG = ", gt_day)
                print("ANON_DAY_BUG = ",anon_day)
...
def mainTest():
    ...
    AT = pd.read_csv('ouput.csv', sep=',', engine='c', na_filter=False, low_memory=False)
    AT.columns = T_COL.values()
    ...
    s1=m.s1_metric()
    slist.append(s1)
    print("S1 score : {}".format(s1))
    s2=m.s2_metric()
    slist.append(s2)
    print("S2 score : {}".format(s2))
    s3=m.s3_metric()
    slist.append(s3)
    print("S3 score : {}".format(s3))
    s4=m.s4_metric()
    slist.append(s4)
    print("S4 score : {}".format(s4))
    s5=m.s5_metric()
    slist.append(s5)
    print("S5 score : {}".format(s5))
    s6=m.s6_metric()
    slist.append(s6)
    print("S6 score : {}".format(s6))

    print("Temps de calcul : {}".format(time.process_time() - start))

    print("Temps de calcul TOTAL : {}".format(time.process_time() - total_time))
    
    print("SCORE TOTAL : {}".format((max(elist)+max(slist))/2))
